...
import { isShadowSourceModel, shadowCallReplacementFor, shadowSourceModelPrefix } from "../../lib/shadow-call";
...
async function handleResponsesInner(
  ...
  try {
    ...
    // Upstream #4102 credential-scope hardening: a shadow rewrite changes the
    // credential domain, so the final auth resolution must strip caller bearer
    // headers, same as combo/policy re-routes.
