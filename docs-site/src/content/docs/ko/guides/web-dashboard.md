...
브라우저에서 `http://localhost:<port>`를 엽니다. 허브에서 관리 리스너를 켜 두면 `http://127.0.0.1:<관리 포트>`를 엽니다. 프록시가 꺼져 있으면 먼저 자동으로 시작합니다.
...
시작 안전성 페이지를 제외한 모든 페이지 상단의 한 줄 요약에 프로바이더별 현재 할당량 사용률이 표시됩니다. 예: `OpenAI 31% | Claude 54% | xAI 12% | Google 8%`. 프로바이더 작업 화면과 같은 할당량 보고(`GET /api/provider-quotas`)를 탭이 보이는 동안 60초마다 읽으며, 업스트림 강제 새로고침은 하지 않습니다.

- 각 항목은 보고된 창 가운데 우선순위가 높은 창을 표시합니다. 주간, 월간, 5시간, 프로바이더 고유 창 또는 선불 크레딧 순서입니다.
- 70% 이상 사용하면 주황색, 90% 이상이면 빨간색으로 표시됩니다.
- 항목에 마우스를 올리거나 키보드로 포커스하면 보고된 모든 창의 사용률, 초기화 시각, 데이터 기준 시각이 나옵니다. 터치 화면에서는 첫 번째 탭에서 나옵니다.
- 항목을 클릭하면(터치 화면에서는 두 번째 탭) 프로바이더 화면에서 해당 프로바이더의 계정 탭이 열리고, 거기서 계정이나 API 키를 관리합니다.
- 요약 바는 항상 한 줄입니다. 항목이 다 들어가지 않으면 옆으로 스크롤하거나 양쪽 끝의 « » 버튼을 누릅니다.
- 할당량 창을 보고하지 않는 프로바이더는 표시하지 않습니다. 보고하는 프로바이더가 없으면 요약 바 전체가 숨겨집니다.
- 오른쪽 끝에 마지막으로 읽은 시각이 표시됩니다. 최근 읽기에 실패해 이전 값을 보여 주는 동안에는 주황색으로 바뀝니다.
...
| **Maintenance** | Codex 모델 카탈로그를 다시 동기화하고, 프로젝트 로컬 설정의 우회 경고를 확인하고, latest/preview 업데이트를 조회하거나 선택적 프록시 재시작과 함께 설치합니다. 데스크톱 셸에서는 업데이트 항목이 패키지 업데이터를 실행하는 대신 네이티브 앱 업데이트 페이지를 엽니다. |
...
| **Models** | 네이티브 GPT와 라우팅 모델을 켜고 끄고, 프로바이더 allowlist와 컨텍스트 상한, v1/base/v2, v2 thread 수를 설정합니다. 이 페이지는 허브에 저장된 카탈로그, 이 클라이언트가 가져온 카탈로그, 실행 중인 클라이언트에서의 활성화를 구분합니다. 가져온 시각만으로 최신 허브 저장 내용이 포함됐다고 확인할 수 없으며, 런타임 활성화는 확인되지 않은 상태로 표시합니다. |
...
- 각 계정 카드에는 **선택 순서** 컨트롤(가장 먼저 / 먼저 / 기본 / 나중에 / 가장 마지막)이 있습니다.
  ...
  있습니다.
- Thread affinity가 요청마다 계정이 흔들리는 일을 막습니다. `pool.cacheAffinity`가 기본으로 켜져
  있으므로, 오래 실행되는 thread는 사용량이 임계값 이상이라는 이유만으로 다시 묶이지 않습니다.
  계정이 소진되었거나 처리할 수 없을 때에만 떠나며, 그때도 실제 quota 여유가 있고 usage가 더 낮은
  계정으로만 옮깁니다. 플래그를 끄면 사용량이 확실히 더 낮고 실제 quota 여유가 있는 정상 계정이
  있을 때 임계값 재바인딩이 복원됩니다.
